...
class TransferSecurityIT extends AbstractTestcontainerBase {
    ...
    @Test
    void transferFromAnotherUsersAccount_shouldReturn403Forbidden() {
        BigDecimal bobBalanceBefore = accountRepository.findById(bobAccountId)
                .orElseThrow().getBalance();

        
        TransferRequestDto request = new TransferRequestDto();
        request.setFromId(bobAccountId);
        request.setToId(aliceAccountId);
        request.setAmount(new BigDecimal("100.00"));

        webTestClient.post()
                .uri("/api/transfers")
                .header("Authorization", "Bearer " + aliceJwt)
                .header("Idempotency-Key", "security-test-" + UUID.randomUUID())
        ...
        BigDecimal bobBalanceAfter = accountRepository.findById(bobAccountId)
                .orElseThrow().getBalance();
        assertThat(bobBalanceAfter)
                .as("Bob's balance must be unchanged after the rejected transfer")
                .isEqualByComparingTo(bobBalanceBefore);
    }
